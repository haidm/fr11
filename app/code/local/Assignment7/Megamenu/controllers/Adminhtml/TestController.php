<?php
class Assignment7_Megamenu_Adminhtml_TestController extends Mage_Adminhtml_Controller_Action
{
    public function indexAction()
    {
        $this->_title($this->__('Sales'))
            ->_title($this->__('New Menu'));

        $this->loadLayout()
            ->_setActiveMenu('sales/test')
            ->_addBreadcrumb(Mage::helper('tax')->__('Sales'), Mage::helper('tax')->__('Sales'))
            ->_addBreadcrumb(Mage::helper('tax')->__('New Menu'), Mage::helper('tax')->__('New Menu'))
        ;

        $this->_addContent($this->getLayout()->createBlock('megamenu/adminhtml_order'))
            ->renderLayout();
        return $this;
    }
    ...
    public function editAction()
    {
        ...
        $this->_addBreadcrumb(
                ...
                $orderId ?  Mage::helper('tax')->__('Edit order') :  Mage::helper('tax')->__('New Order'))
            ->_addContent($this->getLayout()->createBlock('megamenu/adminhtml_order_edit')
